...
bool_exp, bool_term, bool_factor, bool_const = G.NonTerminals('<bool_exp> <bool_term> <bool_factor> <bool_const>')
...
bool_factor %= bool_const, lambda _, s: s[1]

bool_const %= opar_ + bool_exp + cpar_, lambda _, s: s[1]
bool_const %= true_, lambda _, s: BoolNode(s[1])
bool_const %= false_, lambda _, s: BoolNode(s[1])

bool_const %= num_exp + doubleequals_c_ + num_exp, lambda _, s: EqualNode(s[1], s[3], s[2])
bool_const %= str_exp + doubleequals_c_ + str_exp, lambda _, s: EqualNode(s[1], s[3], s[2])

bool_const %= num_exp + different_c_ + num_exp, lambda _, s: DifferenceNode(s[1], s[3], s[2])
bool_const %= str_exp + different_c_ + str_exp, lambda _, s: DifferenceNode(s[1], s[3], s[2])

bool_const %= num_exp + lt_c_ + num_exp, lambda _, s: LessThanNode(s[1], s[3], s[2])
bool_const %= str_exp + lt_c_ + str_exp, lambda _, s: LessThanNode(s[1], s[3], s[2])

bool_const %= num_exp + gt_c_ + num_exp, lambda _, s: GreaterThanNode(s[1], s[3], s[2])
bool_const %= str_exp + gt_c_ + str_exp, lambda _, s: GreaterThanNode(s[1], s[3], s[2])

bool_const %= num_exp + get_c_ + num_exp, lambda _, s: GreaterEqualThanNode(s[1], s[3], s[2])
bool_const %= str_exp + get_c_ + str_exp, lambda _, s: GreaterEqualThanNode(s[1], s[3], s[2])

bool_const %= num_exp + let_c_ + num_exp, lambda _, s: LessEqualThanNode(s[1], s[3], s[2])
bool_const %= str_exp + let_c_ + str_exp, lambda _, s: LessEqualThanNode(s[1], s[3], s[2])
...
str_exp %= opar_ + str_exp + cpar_, lambda _, s: s[2]
